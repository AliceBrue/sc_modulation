...
def net_osim(osim_file, step_size, n_steps, nn_file_name, net_model_file, include, controls_dict, w_pi=-np.ones(7), 
             nn_init=None, integ_acc=0.0001, perturb=None, markers=None, coord_plot=None, musc_acti=None, rates_plot=None, 
             save_folder='results_net/', visualize=False, show_plot=False, save_kin=None):
    """ Integrate and simulate network and osim models """
    ...
    for j in range(1, n_steps):
        ...
        for i in range(n_muscles):
            ...
            if "Ia" in include:
                ...
                # Ia to IaIn
                container.neural.inputs.get_parameter('aff_arm_' + muscle_names[i] + '_IaIn').value = \
                        sc_model.ext_muscles[muscle_names[i]].past_Ia_rates[0]
                # Ia to Mn with PI
                if "Ia" in controls.keys():
                    if "PI_Tsianos" in include:
                        container.neural.inputs.get_parameter('aff_arm_' + muscle_names[i] + '_Ia').value = \
                            max(sc_model.ext_muscles[muscle_names[i]].past_Ia_rates[0] + w_pi[i]*controls["Ia"][i, j], 0)
                    elif "PI_lin_wIa" in include:
                        container.neural.inputs.get_parameter('aff_arm_' + muscle_names[i] + '_Ia').value = \
                            sc_model.ext_muscles[muscle_names[i]].past_Ia_rates[0]*(1 + w_pi[i]*controls["Ia"][i, j])
                # Ia to Mn without PI
                else:
                    container.neural.inputs.get_parameter('aff_arm_' + muscle_names[i] + '_Ia').value = \
                            sc_model.ext_muscles[muscle_names[i]].past_Ia_rates[0]
                ...
                for r in range(len(rates_plot[1])):
                    if rates_plot[1][r] == 'Mn':
                        ...
                        rates[m, r, j-1] = net_.container.neural.inputs.values[
                            net_.container.neural.inputs.names.index('aff_arm_'+rates_plot[0][m]+'_'+"IaIn")]
